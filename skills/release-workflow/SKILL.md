...
- **Split signing into two steps.** Step 1: sign the commit (`git commit --amend --gpg-sign --no-edit`). Verify success with `git verify-commit HEAD`. Step 2: only then run the remaining commands (force-push, tag, push tag). Never chain the amend with the push and tag in one paste — a passphrase typo on the amend would leave the unsigned commit on the remote after the force-push.
